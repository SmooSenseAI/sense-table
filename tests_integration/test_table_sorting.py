...
class TestTableSorting(BaseTableTestCase):
    def test_horizontal_scroll_after_sorting(self):
        ...
        with playwright_page(headless=True) as page:
            ...
            self.assertFalse(
                self.is_element_in_view(table, column_header),
                f"Column header {column} is initially not in view",
            )

            column_header.scroll_into_view_if_needed()
            self.assertTrue(
                self.is_element_in_view(table, column_header),
                f"Column header {column} should be in view after scrolling",
            )
            ...
            self.assertTrue(
                self.is_element_in_view(table, sort_button), "Sort button should be visible"
            )
            sort_button.click()
            page.wait_for_timeout(500)
            self.assertTrue(
                self.is_element_in_view(table, sort_button),
                "Sort button should stay visible after clicking",
            )
            x_after = pl.get_relative_x_of_column(column)
            logger.info(f"Column {column} x after sorting: {x_after}")
            self.assertEqual(
                x_after, x_before, "Column header after sorting should be at the same position"
            )
    ...
    def test_sorting(self):
        ...
        with playwright_page(headless=True) as page:
            ...
            self.assertListEqual(
                values, expected, f"Column {column} values should be sorted in ascending order"
            )
            ...
            self.assertListEqual(
                values, expected, f"Column {column} values should be sorted in descending order"
            )
