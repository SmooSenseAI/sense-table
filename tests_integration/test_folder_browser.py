...
class TestFolderBrowser(BaseTestCase):
    ...
    def test_readme_file(self):
        ...
        with playwright_page(headless=True) as page:
            # Navigate to the home page first
            page.goto(self.folder_browser_url)
            self.assertEqual(page.locator('div.wmde-markdown').count(), 0, "No readme.md file should be present")
            logger.info(f"No readme.md file and markdown display at the beginning")
            ...
            self.assertTrue("This is a test readme file with" in content_text, "Readme content be rendered")
            logger.info(f"Readme updated now")
