...
class DummyDataGenerator:
    ...
    def generate_arrays(self):
        ...
        return {
            ...
            'url': pa.array([f'https://placehold.co/{300 + i}x{400 - i}.png' for i in range(self.n_rows)], type=pa.string()),
            ...
            **{str(t): self._generate_integer_array(t) for t in [pa.int8(), pa.int16(), pa.int32(), pa.int64(), pa.uint8(), pa.uint16(), pa.uint32(), pa.uint64()]},
            **{str(t): self._generate_float_array(t) for t in [pa.float16(), pa.float32(), pa.float64()]},
            'bool': self._generate_bool_array(),
            'bool_with_nulls': self._generate_bool_array(with_nulls=True),
            'int_with_nulls': pa.array(np.random.choice([None, 1, 2, 3], size=self.n_rows), type=pa.int64()),
